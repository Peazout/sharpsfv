...
namespace sharpsfv
{
    public class Crc32
    {
        ...
        {
            crc ^= SEED;
            while (--length >= 0) crc = crcTable[(crc ^ buffer[offset++]) & 0xFF] ^ (crc >> 8);

            return crc ^ SEED;
        }
        public static uint Update(uint crc, IReadOnlyList<byte> bytes)
        {
            crc ^= SEED;
            try
            {
        ...
        public static uint Update(uint crc, IEnumerable<byte> bytes)
        {
            crc ^= SEED;
